...
class _TimePickerSpinnerState extends State<TimePickerSpinner> {
  ...
  void initState() {
    ...
    hourController = new ScrollController(initialScrollOffset: (currentSelectedHourIndex - 2) * _getItemHeight() ); //- 2, damit das dritte Item initial Highlighted wird
    ...
    secondController = new ScrollController(initialScrollOffset: (currentSelectedSecondIndex - 2) * _getItemHeight() );
    ...
    Widget _spinner = NotificationListener<ScrollNotification>(
      onNotification: (scrollNotification){
        if(scrollNotification is UserScrollNotification){
          ...
          setState(() {
            onUpdateSelectedIndex((controller.offset / _getItemHeight()).round() + 2
            );
          });
        }
        return true;
    ...
    Widget _spinner = NotificationListener<ScrollNotification>(
      onNotification: (scrollNotification){
        if(scrollNotification is UserScrollNotification){
          ...
          setState(() {
            currentSelectedAPIndex = (apController.offset / _getItemHeight()).round();
            isAPScrolling = true;
          });
        }
        return true;
